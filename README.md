...
- `GEMINI_MODEL`：可选，默认 `gemini-3.5-flash-lite`；在当前免费额度中具备更宽裕的 RPM 与 RPD，负责精炼医学沟通。
- `GEMINI_FALLBACK_MODEL`：可选，默认 `gemini-3.1-flash-lite`；主模型返回 429 时自动尝试该备用 Gemini 模型。旧的 `GEMINI_PLANNER_MODEL` 配置仍兼容。
...
- **一次 AI 精炼**：确定性路由先识别问题类型、完成急症分诊并选择证据工具；正常回答只调用一次 Gemini，降低延迟和免费额度消耗。主模型返回 429 时才尝试备用 Gemini 模型。
- **先算后说**：工具层先计算最近 7 天平均血压、晨晚差、心率、钠摄入、晚餐时间和睡眠，再把受控结果交给回答生成器，不让模型直接从散乱原始数据猜结论。
- **结构化生成**：Gemini 只生成规定 JSON，包括直接回答、个性化解释、行动建议、安全提醒、数据依据和后续追问。
